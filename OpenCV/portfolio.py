...
[메인 실행 흐름]
- 카메라 프레임을 읽고 전처리한 뒤 움직임 마스크를 생성합니다.
- 상태값(state_now)으로 메뉴/카운트다운/게임/결과/설정 화면을 전환합니다.
- 버튼 선택과 과일 히트는 ROI 움직임 비율과 픽셀 수로 판정합니다.

[핵심 구성]
- Target / TargetPool: 타겟 상태와 재등장 관리
- ImageTool / DrawTool: 이미지 로드와 화면 출력
- MotionTool / MenuTool: 움직임 분석과 입력 상태 초기화
"""
...
class Target:
    ...
    def deactivate_target(self, now_time):
        # 히트되면 비활성으로 전환하고
        self.active_flag = False
        # 랜덤 지연 뒤 다시 나오게 예약(패턴이 너무 규칙적이지 않게 만듦)
        self.respawn_time = now_time + random.uniform(0.25, 0.85)


# 타겟 여러 개를 "묶어서" 관리하는 풀 클래스
# 생성/삭제 대신 활성/비활성만 바꿔 재사용하는 구조(가벼운 풀링 개념)
class TargetPool:
    def __init__(self, target_list):
        self.target_list = target_list

    def spawn_targets(self, active_count, frame_width, frame_height, now_time=None):
        # 앞쪽 active_count 개만 활성화, 나머지는 비활성화
        # 게임 시작/종료 시점에 전체 타겟 상태를 빠르게 맞추기 위한 함수
        for idx, target_obj in enumerate(self.target_list):
            if idx < active_count:
        ...
        if state_now == state_menu:
            ...
            for target_obj in target_pool.list_activeTargets():
                ...
                if hit_ratio > hit_ratio_min and hit_pixels > 15 and cooldown_ok:
                    ...
                    target_obj.deactivate_target(now_time)
