#!/usr/bin/env python3
"""Server_admin_Web.py - Final_Ver02 (Final_Ver01 기반, AMR 양방향 신호 버전)

이 서버는 0723_AMR_Project/Final_Ver02/Arduino/AMR.ino (V16,
AMR_Server_Bridge_V16_ARDUINO_CONVEYOR_AUTO_2_CYCLES) 펌웨어를 대상으로
설계되었습니다. 이전에 참고했던 DistanceSetting_WiFi/AMR_Arduino.ino와는
명령/이벤트 프로토콜이 다르므로 반드시 이 펌웨어 기준으로 읽어야 합니다.
...
로그를 서버가 읽어서 출고 컨베이어를 AMR 도착 조건에 건다":
  ...
  ReadyForPickup -> WaitForAmrArrived -> ConveyDone, 그대로 유지). 서버
  쪽은 ABB의 "ReadyForPickup"과 AMR의 위 STATUS CONVEYOR_START 로그라는
