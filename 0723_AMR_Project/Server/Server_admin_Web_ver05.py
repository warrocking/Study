#!/usr/bin/env python3
"""Server_admin_Web_ver05.py (Final_Ver04/Server/Server_admin_Web.py 기반)

ver04 대비 이 버전에서 바뀐 것 (수동 Start가 픽업 상태를 안 비우던 버그 수정):
  _clear_pickup_state()로 스테이션의 ReadyForPickup/AMR 도착 큐를 비우는 게
  예전엔 _run_production()(=웹 화면 "자동 생산 시작" 버튼) 안에서만
  실행됐다. 그런데 실제로는 "자동 생산 시작"을 거치지 않고 웹 화면의
  "개별 명령"이나 quick 버튼으로 3abb/5abb/amr에 수동으로 Start를 보내는
  경우가 많았는데, 그러면 이전 실행(중간에 끊겼거나 수동으로 재시도했던
  테스트 포함)에서 큐에 남아있던 신호가 전혀 안 지워진 채로 남는다. 그
  상태에서 다시 Start를 보내면, 이번 실행에서 AMR이 실제로 그 스테이션에
  도착하지도 않았는데 예전에 남아있던 "AMR 도착" 신호와 즉시 짝지어져서
  컨베이어가 먼저 돌아버린다("AMR이 5abb에 있는데 3abb가 출고했다" 증상의
  실제 원인으로 확인됨). 이제 send()가 Start/START/RESET_SEQUENCE 전송마다
  공통으로 이 정리를 하므로, 수동으로 보내든 자동 생산으로 보내든 항상
  깨끗한 상태에서 시작한다.
...
#   "몸체를 실어 간다"는 뜻이고 direction=REV는 반납/하차 동작이라 대상이 아니다.
...
# 추측할 필요가 없다. direction=FWD와 함께 station=3abb 또는 station=5abb가
# 있는 경우에만 매칭한다 - station=4abb(ST2 재적재, 이것도 direction=FWD임)는
# 여기 없으므로 자연히 제외된다.
