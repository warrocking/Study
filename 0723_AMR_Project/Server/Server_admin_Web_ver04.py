#!/usr/bin/env python3
"""Server_admin_Web_ver04.py (Server_admin_Web_ver03.py 기반)

ver03 대비 이 버전에서 바뀐 것 - AMR과의 통신을 실제 펌웨어(AMR.ino V16,
AMR_Server_Bridge_V16_ARDUINO_CONVEYOR_AUTO_2_CYCLES,
0723_AMR_Project/Final_Ver02/Arduino/AMR.ino에 사본 있음) 프로토콜에 맞게
전면 재작성하고, 출고 컨베이어를 AMR 도착 확인 후로 게이팅합니다:
...
로그를 서버가 읽어서 출고 컨베이어를 AMR 도착 조건에 건다":
  ...
  3abb_V13.mod/5abb_V16.mod에 ReadyForPickup -> WaitForAmrArrived ->
  ConveyDone 대기를 새로 추가). 서버 쪽은 ABB의 "ReadyForPickup"과 AMR의 위
  STATUS CONVEYOR_START 로그라는
  서로 다른 두 이벤트 스트림을 스테이션별로 매칭하는 백그라운드 스레드
  (_run_pickup_matcher)가 담당합니다 - 어느 쪽이 먼저 도착하든 큐에 쌓아
  두고 매칭되는 즉시 그 스테이션에 "AmrArrived"를 보냅니다.
...
실행:
    ...
    python Server_admin_Web_ver04.py
