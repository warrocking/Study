#!/usr/bin/env python3
"""Server_admin_Web_ver02.py (Server_admin_Web_AMR.py 기반)

AMR 담당자가 완성한 AMR 명령 매핑(RUN ROUTE_STx FWD/REV, RESET_SEQUENCE, arrived)은
그대로 두고, 여기에 UDP 자동 탐색 응답 기능만 추가한 버전입니다.

왜 필요한가: 지금 Arduino(UNO R4 WiFi) 펌웨어는 관리자 서버 주소를
IPAddress ADMIN_SERVER_IP(192, 168, 0, 173) 로 하드코딩해서 접속합니다. 이
노트북의 IP가 바뀌면(DHCP 재할당, 다른 Wi-Fi 등) Arduino가 더는 서버를 못
찾습니다. 그래서 서버가 UDP 5001에서 "누구 없나요" 질문을 기다리다가, 질문이
오면 "나 여기 있고 TCP는 5000번이야" 라고 응답해주는 기능을 추가했습니다.
응답 패킷에는 이 노트북의 IP를 문자열로 직접 넣지 않습니다 - UDP 응답을 보낼 때
운영체제가 채워주는 발신 주소(소켓이 실제로 응답을 내보낸 인터페이스의 주소)를
Arduino가 그대로 서버 IP로 쓰기 때문에, 인터페이스가 여러 개(Wi-Fi/Tailscale/
이더넷)라도 항상 올바른 주소로 응답됩니다.

주의: 이 파일은 서버(파이썬) 쪽만 바꿉니다. 지금 전달받은
AMR_Server_Bridge.ino에는 이 UDP 탐색을 실제로 사용하는 클라이언트 코드가
아직 없습니다(ADMIN_SERVER_IP가 여전히 하드코딩되어 있음) - 그쪽도 함께
바뀌어야 "IP가 바뀌어도 자동으로 찾아 붙는다"는 목적이 실제로 완성됩니다.

Server_admin_Web.py / Server_admin_GUI.py와 네트워킹/오케스트레이션
로직(AdminServer)은 동일합니다. Flask + Server-Sent Events(SSE)로 브라우저에
실시간으로 이벤트를 밀어 넣는 것도 동일합니다.

  - 로봇/AMR 중계기들은 지금까지와 동일하게 TCP 포트 {PORT}로 접속합니다.
  - 사람은 웹 포트 {WEB_PORT}로 브라우저에서 접속합니다.
  - AMR Arduino가 RUN ROUTE_STx FWD/REV 명령을 받아 LD-90 ARCL을 실행합니다.
  - Arduino가 UDP {UDP_PORT}로 "AMR_SERVER_DISCOVER_V1 device=amr"을 보내면,
    이 서버가 보낸 주소로 "AMR_SERVER_V1 name=admin tcp_port={PORT}"를 응답합니다.
...
class AdminServer:
    """네트워킹 + 자동 생산 오케스트레이션 + AMR UDP 자동 탐색 응답. 화면(Flask/
    브라우저)은 전혀 모른다 - 전부 event_queue(Broadcaster)로만 알린다."""
...
<main>
  ...
  <section class="panel">
    <h2>자동 생산 (3&rarr;4&rarr;3&rarr;5&rarr;4&rarr;5 반복, 상부 항상 먼저)</h2>
    <div class="row">
      <label class="hint">목표 대수</label>
      <input id="car-count" type="number" min="1" value="2" style="width:70px">
...
async function startProduction() {
  const count = parseInt(document.getElementById('car-count').value, 10);
  if (!count || count <= 0) { alert('목표 대수는 1 이상의 정수여야 합니다.'); return; }
  const result = await postJson('/api/produce', {count});
  if (!result.ok) alert(result.error);
}
...
def api_produce():
    ...
    try:
        ...
        return jsonify(ok=False, error="목표 대수는 1 이상의 정수여야 합니다."), 400
    ok, error = server.start_production(count)
    return jsonify(ok=ok, error=error)
