#!/usr/bin/env python3
"""Server_admin_Web_ver06.py (ver05 기반)

ver05 대비 이 버전에서 바뀐 것 (ReadyForPickup/AMR 도착을 순서가 아니라
번호로 직접 비교):
  지금까지는 3abb/5abb의 ReadyForPickup과 amr의 도착 이벤트를 각각 큐에
  쌓아두고 순서(FIFO)로만 짝지었다. 로봇이 한 번에 한 유닛만 기다리고
  Start마다 큐를 비우므로 순서 매칭 자체는 구조적으로 안전하지만, "지금
  이게 1번째 차 짝인지 2번째 차 짝인지" 로그로는 확인할 방법이 없었다.
  RAPID(3abb.mod/5abb.mod)가 이제 ReadyForPickup에 unit=N을 붙여 보내고,
  서버는 그 unit=N을 amr STATUS 메시지의 cycle=N과 직접 비교해서
  "번호 확인됨: unit=1 <-> amr cycle=1 일치" 같은 로그를 남긴다. 번호가
  다르면(원칙적으로 발생하면 안 됨) "err" 태그로 눈에 띄게 경고하되,
  생산 라인이 멈추지 않도록 AmrArrived는 그대로 전달한다.
...
class AdminServer:
    ...
    def _run_pickup_matcher(self, station: str) -> None:
        ...
        while self.running.is_set():
            ...
            if ready_unit is not None and arrived_cycle is not None and ready_unit != arrived_cycle:
                self.log(
