...
/*
  Arduino UNO R4 WiFi - AMR bridge V24 EXPLICIT-STATION-SIGNALS
  ...
  Conveyor order per cycle:
    ...
    3. ST1 REV until the previously detected plate clears D2.
    4. ST3 FWD continuously until D2 detects the plate.
    5. ST2 REV 6 s, stop 10 s, then FWD until a new D2 detection.
    6. ST3 REV until the previously detected plate clears D2.
...
const char FIRMWARE_VERSION[] = "AMR_Server_Bridge_V24_EXPLICIT_STATION_SIGNALS";
...
const ProcessStep PROCESS_SEQUENCE[] = {
  ...
  {"ROUTE_ST1", ACTION_UNLOAD_UNTIL_D2_CLEAR},
  {"ROUTE_ST3", ACTION_LOAD_UNTIL_D2},
  {"ROUTE_ST2", ACTION_ST2_UNLOAD_WAIT_RELOAD},
  {"ROUTE_ST3", ACTION_UNLOAD_UNTIL_D2_CLEAR},
};
...
const char* actionName(StationAction action) {
  ...
  if (action == ACTION_UNLOAD_UNTIL_D2_CLEAR) return "UNLOAD_UNTIL_D2_CLEAR";
...
void startLoadUntilD2(bool requireClearBeforeDetection) {
  ...
  // Keep the legacy tags accurate for the current server while also adding
  // explicit station/cycle/step fields for the next server version.
  const char* mode =
...
void startUnloadUntilD2Clear() {
  ...
  sendAdminFormat(
      "STATUS CONVEYOR_START direction=REV mode=UNTIL_D2_CLEAR station=%s cycle=%u step=%u route=%s sensorArmed=%u",
      stationNameForStep(expectedStep),
      static_cast<unsigned int>(completedCycles + 1),
      static_cast<unsigned int>(expectedStep + 1), activeRoute,
...
void startDockAction() {
  ...
  if (action == ACTION_LOAD_UNTIL_D2) {
  ...
  } else if (action == ACTION_UNLOAD_UNTIL_D2_CLEAR) {
    startUnloadUntilD2Clear();
  } else {
    ...
    unsigned long reverseMs =
        action == ACTION_ST2_UNLOAD_WAIT_RELOAD
            ...
            : RETURN_REV_RUN_MS;
    sendAdminFormat(
        "STATUS CONVEYOR_START direction=REV mode=ST2_UNLOAD_TIMER station=%s cycle=%u step=%u route=%s timer_ms=%lu",
        stationNameForStep(expectedStep),
        static_cast<unsigned int>(completedCycles + 1),
