...
/*
  final_V01 - server-gated AMR production sequencer

  Arduino UNO R4 WiFi bridge for:
    Main admin server <-> Arduino <-> Omron LD-90 ARCL

  Production policy:
    - "Start" starts only step 1 and moves to 3abb (MACRO_ST1).
    - The Arduino never advances a production step by itself.
    - After each completed macro it reports ARRIVED and READY_FOR_NEXT, then waits.
    - The server advances the sequence with Next or an explicit Goto3abb/4abb/5abb.
    - Required sequence: 3 -> 4 -> 3 -> 5 -> 4 -> 5, repeated twice (12 steps).

  Retained safety and device features:
    - Matches the admin TCP protocol (port 5000, first line "amr").
    - Finds the current admin laptop address by UDP discovery.
...
// The laptop address is discovered at runtime. It is not hard-coded.
...
const char FIRMWARE_VERSION[] = "2026-08-07-final-v01-server-gated";
...
// Start begins step 1. Every later step requires a new server command.
...
const char *const PRODUCTION_MACROS[] = {
  MACRO_ST1, MACRO_ST2, MACRO_ST1, MACRO_ST3, MACRO_ST2, MACRO_ST3,
  MACRO_ST1, MACRO_ST2, MACRO_ST1, MACRO_ST3, MACRO_ST2, MACRO_ST3
};

const ConveyorDirection PRODUCTION_CONVEYOR[] = {
  CONVEYOR_FORWARD,
  CONVEYOR_REVERSE,
  CONVEYOR_REVERSE,
  CONVEYOR_FORWARD,
...
const char *const PRODUCTION_LABELS[] = {
  "UPPER_PICKUP_AT_3ABB_R1",
  "UPPER_DROPOFF_AT_4ABB_R1",
  "UPPER_RETURN_AT_3ABB_R1",
  "LOWER_PICKUP_AT_5ABB_R1",
  "LOWER_DROPOFF_AT_4ABB_R1",
  "LOWER_RETURN_AT_5ABB_R1",
  "UPPER_PICKUP_AT_3ABB_R2",
  "UPPER_DROPOFF_AT_4ABB_R2",
  "UPPER_RETURN_AT_3ABB_R2",
  "LOWER_PICKUP_AT_5ABB_R2",
  "LOWER_DROPOFF_AT_4ABB_R2",
  "LOWER_RETURN_AT_5ABB_R2"
};

const uint8_t STEPS_PER_REPEAT = 6;
const uint8_t PRODUCTION_REPEAT_COUNT = 2;
const uint8_t PRODUCTION_STEP_COUNT =
    sizeof(PRODUCTION_MACROS) / sizeof(PRODUCTION_MACROS[0]);

static_assert(PRODUCTION_STEP_COUNT ==
                  STEPS_PER_REPEAT * PRODUCTION_REPEAT_COUNT,
              "Production sequence size mismatch");
...
void sendStatus(const char *prefix) {
  ...
  reportAdmin(
    ...
    "MOTORS=%s FAULT=%u FAULT_SCAN=%s MOTION=%s ACTIVE=%s PRODUCTION=%u "
    "STEP=%u/%u NEXT_READY=%u HUSKY=%s CONVEYOR=%s uptime_ms=%lu rssi_dbm=%ld",
    prefix,
    FIRMWARE_VERSION,
    ...
    static_cast<unsigned int>(PRODUCTION_STEP_COUNT),
    nextMacroPending ? 1 : 0,
...
bool motionAllowed(bool requireHusky) {
  if (stopRequested) {
    reportAdmin("ERR STOP_PENDING wait=STOP_CONFIRMED");
    return false;
  }
...
bool beginProductionStep() {
  if (!cycleRunning) {
    reportAdmin("ERR PRODUCTION_NOT_STARTED use=Start");
    return false;
  }
  if (cycleStep >= PRODUCTION_STEP_COUNT) {
    reportAdmin("ERR PRODUCTION_STEP_OUT_OF_RANGE step=%u",
                static_cast<unsigned int>(cycleStep + 1));
    return false;
  }

  nextMacroPending = false;
  if (!beginMacro(PRODUCTION_MACROS[cycleStep], true)) {
    nextMacroPending = true;
    reportAdmin("ERR PRODUCTION_STEP_REJECTED step=%u macro=%s",
                static_cast<unsigned int>(cycleStep + 1),
                PRODUCTION_MACROS[cycleStep]);
    return false;
  }

  arrivalRequired = true;
  copyText(activeArrivalLabel, sizeof(activeArrivalLabel),
           PRODUCTION_LABELS[cycleStep]);
  reportAdmin("EVENT PRODUCTION_LEG_REQUESTED step=%u/%u repeat=%u station=%s macro=%s",
              static_cast<unsigned int>(cycleStep + 1),
              static_cast<unsigned int>(PRODUCTION_STEP_COUNT),
              static_cast<unsigned int>(cycleStep / STEPS_PER_REPEAT + 1),
              activeArrivalLabel, activeMotionName);
  return true;
}
...
void startCycle() {
  ...
  nextMacroPending = false;
  reportAdmin(
    "EVENT PRODUCTION_START repeats=2 sequence=3ABB,4ABB,3ABB,5ABB,4ABB,5ABB mode=SERVER_GATED");

  if (!beginProductionStep()) {
    clearProcessState();
    reportAdmin("ERR PRODUCTION_ABORT reason=FIRST_MACRO_REJECTED");
  }
}

ConveyorDirection directionForCurrentMacro() {
  const uint8_t step = cycleRunning ? cycleStep : manualSequenceStep;
  const uint8_t expectedStep = cycleRunning ? step : step % STEPS_PER_REPEAT;
  if (expectedStep >= PRODUCTION_STEP_COUNT) return CONVEYOR_STOPPED;
  if (!equalsIgnoreCase(activeMotionName, PRODUCTION_MACROS[expectedStep])) {
    reportAdmin("ERR SEQUENCE_MISMATCH expected=%s actual=%s step=%u",
                PRODUCTION_MACROS[expectedStep], activeMotionName,
                static_cast<unsigned int>(expectedStep + 1));
    return CONVEYOR_STOPPED;
  }
  return PRODUCTION_CONVEYOR[expectedStep];
}
...
void finishMacro() {
  ...
  if (wasCycle && (!waitWasSeen || !conveyorSucceeded)) {
    abortProcess(!waitWasSeen ? "PRODUCTION_WAIT_NOT_SEEN" : "PRODUCT_NOT_DETECTED");
    return;
  }
  ...
  if (!wasCycle) {
    if (waitWasSeen && conveyorSucceeded) {
      manualSequenceStep = (manualSequenceStep + 1) % STEPS_PER_REPEAT;
    }
  ...
  if (mustReportArrival) {
    reportAdmin("ARRIVED leg=%s macro=%s step=%u/%u",
                arrivalLabel, completed,
                static_cast<unsigned int>(cycleStep + 1),
                static_cast<unsigned int>(PRODUCTION_STEP_COUNT));
  }

  reportAdmin("EVENT PRODUCTION_STEP_COMPLETE step=%u/%u macro=%s",
              static_cast<unsigned int>(cycleStep + 1),
              static_cast<unsigned int>(PRODUCTION_STEP_COUNT), completed);
  cycleStep++;
  ...
  if (cycleStep >= PRODUCTION_STEP_COUNT) {
    ...
    cycleStep = 0;
    nextMacroPending = false;
    reportAdmin("DONE PRODUCTION repeats=%u sequence_steps=%u",
                static_cast<unsigned int>(PRODUCTION_REPEAT_COUNT),
                static_cast<unsigned int>(PRODUCTION_STEP_COUNT));
    return;
  }

  // This flag means "waiting for the server", not "start after a timer".
  nextMacroPending = true;
  reportAdmin("READY_FOR_NEXT step=%u/%u expected_station=%s expected_macro=%s",
              static_cast<unsigned int>(cycleStep + 1),
              static_cast<unsigned int>(PRODUCTION_STEP_COUNT),
              PRODUCTION_LABELS[cycleStep], PRODUCTION_MACROS[cycleStep]);
}
...
void requestNextProductionStep(const char *requestedMacro,
                               const char *commandName) {
  if (!cycleRunning) {
    reportAdmin("ERR PRODUCTION_NOT_STARTED command=%s use=Start", commandName);
    return;
  }
  if (motionKind != MOTION_NONE) {
    reportAdmin("ERR AMR_BUSY command=%s active=%s", commandName,
                activeMotionName);
    return;
  }
  if (!nextMacroPending) {
    reportAdmin("ERR NEXT_NOT_READY command=%s step=%u", commandName,
                static_cast<unsigned int>(cycleStep + 1));
    return;
  }
  if (cycleStep >= PRODUCTION_STEP_COUNT) {
    reportAdmin("ERR PRODUCTION_STEP_OUT_OF_RANGE command=%s", commandName);
    return;
  }
  if (requestedMacro != nullptr &&
      !equalsIgnoreCase(requestedMacro, PRODUCTION_MACROS[cycleStep])) {
    reportAdmin("ERR WRONG_DESTINATION command=%s expected=%s received=%s step=%u",
                commandName, PRODUCTION_MACROS[cycleStep], requestedMacro,
                static_cast<unsigned int>(cycleStep + 1));
    return;
  }
...
void handleAdminCommand(char *command) {
  ...
  if (equalsIgnoreCase(command, "start") ||
      equalsIgnoreCase(command, "cycle")) {
    startCycle();
  } else if (equalsIgnoreCase(command, "next")) {
    requestNextProductionStep(nullptr, "Next");
  } else if (equalsIgnoreCase(command, "Goto3abb")) {
    requestNextProductionStep(MACRO_ST1, "Goto3abb");
  } else if (equalsIgnoreCase(command, "Goto4abb")) {
    requestNextProductionStep(MACRO_ST2, "Goto4abb");
  } else if (equalsIgnoreCase(command, "Goto5abb")) {
    requestNextProductionStep(MACRO_ST3, "Goto5abb");
  } else if (equalsIgnoreCase(command, "route1")) {
    ...
    requestNextProductionStep(MACRO_ST1, "GotoUpperPickup");
  } else if (equalsIgnoreCase(command, "GotoDropoffUpper")) {
    requestNextProductionStep(MACRO_ST2, "GotoDropoffUpper");
  } else if (equalsIgnoreCase(command, "GotoUpperReturn")) {
    requestNextProductionStep(MACRO_ST1, "GotoUpperReturn");
  } else if (equalsIgnoreCase(command, "GotoLowerPickup")) {
    requestNextProductionStep(MACRO_ST3, "GotoLowerPickup");
  } else if (equalsIgnoreCase(command, "GotoDropoffLower")) {
    requestNextProductionStep(MACRO_ST2, "GotoDropoffLower");
  } else if (equalsIgnoreCase(command, "GotoLowerReturn")) {
    requestNextProductionStep(MACRO_ST3, "GotoLowerReturn");
    ...
    reportAdmin(
      "ERR UNKNOWN_COMMAND use=Prepare,Arm,Start,Next,Goto3abb,Goto4abb,Goto5abb,Stop,Status,Route1-3,Macro1-3,Reset_Sequence,Conveyor_Forward,Conveyor_Reverse,Conveyor_Off");
  }
}
...
IPAddress broadcastAddress() {
  ...
  return IPAddress(
      local[0] | static_cast<uint8_t>(~SUBNET_MASK[0]),
      local[1] | static_cast<uint8_t>(~SUBNET_MASK[1]),
      local[2] | static_cast<uint8_t>(~SUBNET_MASK[2]),
      local[3] | static_cast<uint8_t>(~SUBNET_MASK[3]));
}

bool isSameLocalNetwork(const IPAddress &candidate) {
  const IPAddress local = WiFi.localIP();
  for (uint8_t i = 0; i < 4; ++i) {
    if ((candidate[i] & SUBNET_MASK[i]) != (local[i] & SUBNET_MASK[i])) {
      return false;
    }
  }
  return true;
...
void startDiscoveryUdp() {
  ...
  if (!discoveryUdpStarted) {
    Serial.println("ERR DISCOVERY_UDP_START_FAILED port=5002");
  }
}

void maintainAdminDiscovery() {
  if (!wifiOnline || adminServerKnown || adminOnline) return;
  if (!discoveryUdpStarted) startDiscoveryUdp();
  if (!discoveryUdpStarted) return;

  const unsigned long now = millis();
  if (now - lastDiscoveryAttempt < DISCOVERY_RETRY_MS) return;
  lastDiscoveryAttempt = now;

  const IPAddress broadcast = broadcastAddress();
  if (!discoveryUdp.beginPacket(broadcast, DISCOVERY_SERVER_PORT)) return;
  discoveryUdp.print(DISCOVERY_REQUEST);
  discoveryUdp.endPacket();
  Serial.println("STATUS ADMIN_DISCOVERY_SENT udp=5001");
}

void readDiscoveryResponses() {
  if (!discoveryUdpStarted || adminServerKnown || adminOnline) return;
  ...
  while (packetSize > 0) {
    ...
    while (discoveryUdp.available()) {
      ...
      if (value < 0) break;
      if (stored < static_cast<int>(sizeof(reply) - 1)) {
        reply[stored++] = static_cast<char>(value);
      }
    }
    reply[stored] = '\0';
    trimAscii(reply);
    ...
    if (equalsIgnoreCase(reply, DISCOVERY_RESPONSE) &&
        isSameLocalNetwork(sender)) {
      adminServerIP = sender;
      adminServerKnown = true;
      lastAdminAttempt = millis() - ADMIN_RETRY_MS;
      Serial.print("EVENT ADMIN_DISCOVERED ip=");
      Serial.println(adminServerIP);
      return;
    }
...
void maintainWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED && WiFi.localIP() == ARDUINO_IP) {
    ...
    lastDiscoveryAttempt = now - DISCOVERY_RETRY_MS;
    adminServerKnown = false;
    startDiscoveryUdp();
...
void attemptAdminConnection() {
  ...
  Serial.print(adminServerIP);
  Serial.print(':');
  Serial.println(ADMIN_SERVER_PORT);

  if (!adminClient.connect(adminServerIP, ADMIN_SERVER_PORT)) {
    adminServerKnown = false;
    return;
  }
  ...
  // The admin server requires the device name as the first line.
  ...
  const IPAddress ip = WiFi.localIP();
  reportAdmin(
    "ONLINE device=amr firmware=%s wifi_ip=%u.%u.%u.%u admin_ip=%u.%u.%u.%u "
    "admin_port=5000 start_action=PRODUCTION_STEP_1 mode=SERVER_GATED repeats=2",
    FIRMWARE_VERSION, ip[0], ip[1], ip[2], ip[3],
    adminServerIP[0], adminServerIP[1], adminServerIP[2], adminServerIP[3]);
  sendStatus("STATUS");
}
...
void maintainAdminConnection() {
  ...
  if (adminServerKnown && millis() - lastAdminAttempt >= ADMIN_RETRY_MS) {
...
void setup() {
  ...
  Serial.println("final_V01 server-gated AMR starting");
  Serial.print("FIRMWARE ");
  Serial.println(FIRMWARE_VERSION);
  Serial.println("Admin: UDP auto-discovery 5001, TCP 5000, device name: amr");
  Serial.println("Production: 3-4-3-5-4-5 x2, server command required per step");
