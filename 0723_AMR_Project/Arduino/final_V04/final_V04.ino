...
void startDockAction() {
  ...
  if (action == ACTION_LOAD_UNTIL_D2) {
    // v26: 3abb/5abb의 "새로 싣기"도 4abb 재적재(ACTION_ST2_UNLOAD_WAIT_RELOAD)
    // 와 똑같이 "센서가 먼저 한 번 꺼졌다가 새로 켜지는" 진짜 새 감지만
    // 인정하도록 통일했다. false였을 때는 도킹 직후 D2가 (이전 물체가
    // 아직 안 지워져서) 우연히 이미 켜져 있으면 실제로 새로 싣는 걸
    // 기다리지도 않고 바로 완료 처리해버리는 문제가 있었다 - 소프트웨어
    // 로그(direction=FWD, AmrArrived 등)는 전부 정상으로 보이는데 AMR
    // 자신은 물건을 제대로 안 받고 떠나버릴 수 있는 위험한 비대칭이었다.
