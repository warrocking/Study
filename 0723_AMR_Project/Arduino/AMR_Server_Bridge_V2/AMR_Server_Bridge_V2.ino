...
/*
  AMR_Server_Bridge_V2

  Arduino UNO R4 WiFi bridge for the current AMR integration package:

    Server_admin_V3_AMR.py / Server_admin_Web_AMR.py
                 TCP 5000
                     |
              Arduino UNO R4 WiFi
                     |
...
// The central laptop's current ROBOT_MA2_2G address.
...
const char FIRMWARE_VERSION[] = "2026-08-06-amr-server-bridge-v2";
...
void maintainWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    wifiOnline = true;
    lastWiFiCheckAt = now;
    lastAdminAttemptAt = now - ADMIN_RETRY_MS;
...
void attemptAdminConnection() {
  ...
  const IPAddress localIP = WiFi.localIP();
  reportAdmin(
      "ONLINE device=amr firmware=%s wifi_ip=%u.%u.%u.%u admin=192.168.0.168:5000 protocol=RUN_ROUTE_V2",
...
void maintainAdminConnection() {
  ...
  if (millis() - lastAdminAttemptAt >= ADMIN_RETRY_MS) {
...
void setup() {
  ...
  Serial.println("AMR_Server_Bridge_V2 starting");
  Serial.print("FIRMWARE ");
  Serial.println(FIRMWARE_VERSION);
  Serial.println("ADMIN 192.168.0.168:5000 name=amr");
