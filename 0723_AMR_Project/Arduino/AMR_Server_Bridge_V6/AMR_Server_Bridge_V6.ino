...
/*
  Arduino UNO R4 WiFi - AMR bridge V6

  Merge of two independently-written versions (V3, close to the AMR owner's
  original sketch; V4, a more elaborate rewrite) plus real-world fixes:

  V6 change: ARCL_PASSWORD corrected from "adept" (wrong, leftover from an
  earlier/different setup) to "1234" (confirmed by the AMR owner as the real
  MobilePlanner ARCL Server Setup password). Everything else is unchanged
  from V5 - see "What changed in V5 and why" below.
...
const char ARCL_PASSWORD[] = "1234";  // confirmed by AMR owner (was wrongly "adept")

const char FIRMWARE_VERSION[] = "AMR_Server_Bridge_V6";
