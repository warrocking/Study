...
/*
  Arduino UNO R4 WiFi - AMR bridge V16 ARDUINO-CONVEYOR AUTO-2-CYCLES
  ...
  Conveyor order per cycle:
    ...
    2. ST2 REV 6 s, stop 10 s, then FWD 6 s.
    3. ST1 REV 5 s.
    4. ST3 FWD 5 s.
    5. ST2 REV 6 s, stop 10 s, then FWD 6 s.
    6. ST3 REV 5 s.
...
const char FIRMWARE_VERSION[] = "AMR_Server_Bridge_V16_ARDUINO_CONVEYOR_AUTO_2_CYCLES";
...
const ProcessStep PROCESS_SEQUENCE[] = {
  ...
  {"ROUTE_ST1", ACTION_UNLOAD_5S},
  {"ROUTE_ST3", ACTION_LOAD_FWD_5S},
  {"ROUTE_ST2", ACTION_ST2_UNLOAD_WAIT_RELOAD},
  {"ROUTE_ST3", ACTION_UNLOAD_5S},
};
...
void beginRoute(const char* route, ConveyorDirection direction) {
  ...
  sendAdminFormat("STATUS ROUTE_START cycle=%u step=%u route=%s direction=%s workflow=%s",
...
void finishDockTransfer() {
  ...
  sendAdminFormat("STATUS TRANSFER_DONE cycle=%u step=%u next=CANCEL_WAIT",
                  static_cast<unsigned int>(completedCycles + 1),
                  static_cast<unsigned int>(expectedStep + 1));
  if (arclClient.connected()) sendArclRaw("waitTaskCancel");
...
void startLoadUntilD2(bool requireClearBeforeDetection) {
  ...
  sendAdminFormat("STATUS CONVEYOR_START direction=FWD mode=UNTIL_D2 sensorArmed=%u",
...
void startDockAction() {
  ...
  if (action == ACTION_LOAD_UNTIL_D2) {
  ...
  } else if (action == ACTION_LOAD_FWD_5S) {
    workflowState = WORKFLOW_FORWARD_TIMED;
    startConveyor(CONVEYOR_FWD);
    sendAdminRaw("STATUS CONVEYOR_START direction=FWD timer_ms=5000 step=4");
  } else {
    ...
    unsigned long reverseMs =
        action == ACTION_ST2_UNLOAD_WAIT_RELOAD
            ...
            : RETURN_REV_RUN_MS;
    sendAdminFormat("STATUS CONVEYOR_START direction=REV timer_ms=%lu step=%u",
                    reverseMs, static_cast<unsigned int>(expectedStep + 1));
  }
}
...
void handleAdminCommand(char* rawCommand) {
  ...
  if (equalsIgnoreCase(command, "START")) {
    ...
    sendAdminRaw("STATUS START_ACCEPTED mode=AUTO_2_CYCLES route=START");
...
void finishActiveRoute() {
  ...
  if (equalsIgnoreCase(completedRoute, "SAFE")) {
    ...
    sendAdminFormat("STATUS SAFE_FINISHED next=%s cycle=%u step=%u",
                    currentStationRoute(),
                    static_cast<unsigned int>(completedCycles + 1),
                    static_cast<unsigned int>(expectedStep + 1));
    beginRoute(currentStationRoute(), CONVEYOR_STOPPED);
...
void handleArclLine(char* rawLine) {
  ...
  // Depending on the ARCL/MobilePlanner version, waitIndefinitely can be
  // reported as either "WaitState: Waiting ..." or simply a state containing
  // "Pausing". Treat both as the same station-docking completion signal.
  bool stationWaitEntered =
      containsIgnoreCase(line, "WaitState: Waiting") ||
      containsIgnoreCase(line, "WaitTaskState: Waiting") ||
      containsIgnoreCase(line, "waitIndefinitely") ||
      containsIgnoreCase(line, "Pausing");
  if (routeRunning && stationWaitEntered &&
...
void pollStationWaitState() {
  if (!routeRunning || workflowState != WORKFLOW_STATION_ROUTE ||
      dockWaitObserved || !arclAuthenticated || !arclClient.connected()) return;
...
void enforceTimeouts() {
  ...
  if (workflowState == WORKFLOW_UNLOADING_TIMED && conveyorRunning &&
    ...
    if (currentAction == ACTION_ST2_UNLOAD_WAIT_RELOAD) {
      ...
      sendAdminRaw("STATUS CONVEYOR_STOP reason=REV_TIMER_5S next=CANCEL_WAIT");
      finishDockTransfer();
    }
  }
