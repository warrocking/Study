...
/*
  Arduino UNO R4 WiFi - AMR bridge V28 INTERRUPTED ROUTE RETRY
  ...
  V26 safety-critical protocol change:
    - PICKUP_ARRIVED is emitted only for a loading visit to 3abb/5abb.
    - RETURN_ARRIVED is emitted only for an empty-plate return visit.
    - TRANSFER_ARRIVED is emitted for the two 4abb transfer visits.
    - PICKUP_ARRIVED carries unit/cycle/run and is retried until the admin
      server replies with an exact ARRIVAL_ACK. Return events are never
      eligible to release a 3abb/5abb outbound conveyor.
...
const char FIRMWARE_VERSION[] = "AMR_Server_Bridge_V28_INTERRUPTED_RETRY";
...
const unsigned long PICKUP_ARRIVAL_RETRY_MS = 2000UL;
const uint8_t MAX_ROUTE_RETRIES = 3;
const unsigned long ROUTE_RETRY_DELAY_MS = 3000UL;
...
bool pendingPickupArrival = false;
char pendingPickupArrivalLine[192] = "";
char pendingPickupArrivalAck[112] = "";
unsigned long lastPickupArrivalSendAt = 0;

bool routeRetryPending = false;
uint8_t routeRetryCount = 0;
char routeRetryName[32] = "";
unsigned long routeRetryScheduledAt = 0;
bool routeRetryIsSafe = false;
...
void resetWorkflow() {
  ...
  pendingPickupArrival = false;
  pendingPickupArrivalLine[0] = '\0';
  pendingPickupArrivalAck[0] = '\0';
  lastPickupArrivalSendAt = 0;
  routeRetryPending = false;
  routeRetryCount = 0;
  routeRetryName[0] = '\0';
  routeRetryScheduledAt = 0;
  routeRetryIsSafe = false;
}
...
void connectAdminIfNeeded() {
  ...
  if (pendingPickupArrival && sendAdminRaw(pendingPickupArrivalLine)) {
    Serial.print("ADMIN_REPLAY ");
    Serial.println(pendingPickupArrivalLine);
    lastPickupArrivalSendAt = millis();
  }
}
...
void scheduleActiveRouteRetry(const char* reason) {
  // Retry only while travelling/docking, before any conveyor transfer starts,
  // or while SAFE itself is moving. Re-running a route after material transfer
  // could repeat the conveyor action and is therefore intentionally rejected.
  bool retryableStation = workflowState == WORKFLOW_STATION_ROUTE;
  bool retryableSafe = workflowState == WORKFLOW_SAFE_ROUTE;
  if (!routeRunning || (!retryableStation && !retryableSafe)) {
    failActiveRoute(reason);
    return;
  }

  if (routeRetryCount >= MAX_ROUTE_RETRIES) {
    failActiveRoute("ROUTE_RETRY_LIMIT_REACHED");
    return;
  }

  stopConveyor();
  strncpy(routeRetryName, activeRoute, sizeof(routeRetryName) - 1);
  routeRetryName[sizeof(routeRetryName) - 1] = '\0';
  routeRetryIsSafe = retryableSafe;
  ++routeRetryCount;
  routeRetryPending = true;
  routeRetryScheduledAt = millis();

  sendAdminFormat(
      "EVENT ROUTE_RETRY_SCHEDULED route=%s attempt=%u/%u delay_ms=%lu detail=%s",
      routeRetryName, static_cast<unsigned int>(routeRetryCount),
      static_cast<unsigned int>(MAX_ROUTE_RETRIES), ROUTE_RETRY_DELAY_MS, reason);

  if (arclClient.connected()) sendArclRaw("stop");
  clearActiveRoute();
  workflowState = routeRetryIsSafe ? WORKFLOW_WAIT_ROUTE_FINISH : WORKFLOW_IDLE;
  dockWaitObserved = false;
  conveyorStartedForRoute = false;
  waitStateClearedForRoute = false;
  waitCancelPending = false;
...
void retryFailedRouteIfDue() {
  if (!routeRetryPending || routeRunning || !sequenceStarted) return;
  if (millis() - routeRetryScheduledAt < ROUTE_RETRY_DELAY_MS) return;
  if (!arclReady || !arclClient.connected() ||
      !motorsCheckComplete || !motorsEnabled ||
      !faultCheckComplete || faultDetected) return;

  char retryRoute[32];
  strncpy(retryRoute, routeRetryName, sizeof(retryRoute) - 1);
  retryRoute[sizeof(retryRoute) - 1] = '\0';
  routeRetryPending = false;
  sendAdminFormat("STATUS ROUTE_RETRY_START route=%s attempt=%u/%u",
                  retryRoute, static_cast<unsigned int>(routeRetryCount),
                  static_cast<unsigned int>(MAX_ROUTE_RETRIES));
  beginRoute(retryRoute, CONVEYOR_STOPPED);
}

void finishDockTransfer() {
  stopConveyor();
  // The local D2/timed transfer has completed, so an old PICKUP_ARRIVED must
  // never continue into SAFE or the following station even if the legacy
  // admin server does not implement ARRIVAL_ACK.
  pendingPickupArrival = false;
  pendingPickupArrivalLine[0] = '\0';
  pendingPickupArrivalAck[0] = '\0';
  lastPickupArrivalSendAt = 0;
...
void publishPickupArrival() {
  snprintf(
      pendingPickupArrivalLine, sizeof(pendingPickupArrivalLine),
      "EVENT PICKUP_ARRIVED station=%s unit=%u cycle=%u step=%u run=%lu route=%s",
      stationNameForStep(expectedStep),
      static_cast<unsigned int>(completedCycles + 1),
      static_cast<unsigned int>(completedCycles + 1),
      static_cast<unsigned int>(expectedStep + 1),
      static_cast<unsigned long>(sequenceRunId), activeRoute);
  snprintf(pendingPickupArrivalAck, sizeof(pendingPickupArrivalAck),
           "ARRIVAL_ACK station=%s unit=%u run=%lu",
           stationNameForStep(expectedStep),
           static_cast<unsigned int>(completedCycles + 1),
           static_cast<unsigned long>(sequenceRunId));
  pendingPickupArrival = true;
  sendAdminRaw(pendingPickupArrivalLine);
  lastPickupArrivalSendAt = millis();
}

void publishPhasedDockArrival(StationAction action) {
  const unsigned int unit = static_cast<unsigned int>(completedCycles + 1);
  const unsigned int step = static_cast<unsigned int>(expectedStep + 1);

  if (action == ACTION_LOAD_UNTIL_D2 &&
      (expectedStep == 0 || expectedStep == 3)) {
    publishPickupArrival();
    return;
  }

  if (action == ACTION_UNLOAD_REV_6S &&
      (expectedStep == 2 || expectedStep == 5)) {
    sendAdminFormat(
        "EVENT RETURN_ARRIVED station=%s unit=%u cycle=%u step=%u run=%lu route=%s",
        stationNameForStep(expectedStep), unit, unit, step,
        static_cast<unsigned long>(sequenceRunId), activeRoute);
    return;
  }

  sendAdminFormat(
      "EVENT TRANSFER_ARRIVED station=%s unit=%u cycle=%u step=%u run=%lu route=%s action=%s",
      stationNameForStep(expectedStep), unit, unit, step,
      static_cast<unsigned long>(sequenceRunId), activeRoute,
      actionName(action));
}
...
void startDockAction() {
  ...
  publishPhasedDockArrival(action);
  if (action == ACTION_LOAD_UNTIL_D2) {
    // v28-fix: 3abb/5abb의 "새로 싣기"도 4abb 재적재(ACTION_ST2_UNLOAD_WAIT_RELOAD)
    // 와 동일하게 "센서가 먼저 한 번 꺼졌다가 새로 켜지는" 진짜 새 감지만
    // 인정하도록 통일했다(Final_Ver07/Arduino/AMR.ino에서 이미 검증된 수정을
    // 이 배포판에도 반영). false였을 때는 도킹 직후 D2가 이전 물체의 잔류
    // 감지로 우연히 이미 켜져 있으면 실제로 새로 싣는 걸 기다리지도 않고
    // 바로 완료 처리해버리는 문제가 있었다 - 소프트웨어 로그(direction=FWD,
    // PICKUP_ARRIVED 등)는 전부 정상으로 보이는데 AMR 자신은 물건을 제대로
    // 안 받고 떠나버릴 수 있는 위험한 비대칭이었다.
...
void handleAdminCommand(char* rawCommand) {
  ...
  if (startsWithIgnoreCase(command, "ARRIVAL_ACK ")) {
    if (!pendingPickupArrival) {
      sendAdminRaw("STATUS ARRIVAL_ACK_DUPLICATE_IGNORED");
    } else if (equalsIgnoreCase(command, pendingPickupArrivalAck)) {
      pendingPickupArrival = false;
      pendingPickupArrivalLine[0] = '\0';
      pendingPickupArrivalAck[0] = '\0';
      lastPickupArrivalSendAt = 0;
      sendAdminFormat("STATUS ARRIVAL_ACK_ACCEPTED ack=%s", command);
    } else {
      sendAdminFormat("ERR ARRIVAL_ACK_MISMATCH received=%s", command);
    }
    return;
  }
  ...
  if (equalsIgnoreCase(command, "START")) {
    ...
    resetWorkflow();
    ++sequenceRunId;
    if (sequenceRunId == 0) ++sequenceRunId;
    sequenceStarted = true;
    sendAdminFormat("EVENT START_ACCEPTED run=%lu mode=AUTO_2_CYCLES cycles=2 steps=12 first_station=3abb route=START",
                    static_cast<unsigned long>(sequenceRunId));
...
void finishActiveRoute() {
  ...
  // A successful route gives the following route its own retry allowance.
  routeRetryPending = false;
  routeRetryCount = 0;
  routeRetryName[0] = '\0';
  routeRetryScheduledAt = 0;
...
void handleArclLine(char* rawLine) {
  ...
  if (routeRunning && containsIgnoreCase(line, "Failed patrolling route")) {
    scheduleActiveRouteRetry(line);
    return;
  }
  if (routeRunning && containsIgnoreCase(line, "Interrupted: Patrolling route")) {
    scheduleActiveRouteRetry(line);
    return;
  }
...
void retryPendingPickupArrival() {
  if (!pendingPickupArrival || !adminClient.connected()) return;
  unsigned long now = millis();
  if (lastPickupArrivalSendAt != 0 &&
      now - lastPickupArrivalSendAt < PICKUP_ARRIVAL_RETRY_MS) return;
  sendAdminRaw(pendingPickupArrivalLine);
  lastPickupArrivalSendAt = now;
}
...
void loop() {
  ...
  retryFailedRouteIfDue();
  monitorConnections();
  retryPendingPickupArrival();
  monitorObjectSensor();
  enforceTimeouts();
