# 3abb_V06.py (3abb_V05.py 기반, 5abb_V08.py와 같은 구조)
#
# 3abb 로봇 옆 노트북에서 실행하는 중계기 프로그램.
# 설정값이 이 파일 안에 이미 채워져 있으므로, 별도 설정 파일 없이 이 파일 하나만
# 그대로 실행하면 됩니다.
#
# V05 대비 이 버전에서 바뀐 것 (관리자 -> ABB 방향에도 버퍼 드레인 추가):
#   V03에서 ABB -> 관리자 방향엔 "한 번의 recv()에 여러 줄이 섞여 들어올 수
#   있다"는 걸 감안해서 드레인 루프를 추가했었는데, 정작 반대 방향(관리자 ->
#   ABB, 즉 "Start"/"AmrArrived" 같은 명령을 로봇에 전달하는 쪽)에는 똑같은
#   드레인 루프가 빠져 있었다. 관리자가 두 명령을 짧은 간격으로 보내서 커넥터
#   쪽 recv() 한 번에 같이 들어오면, read_ready_line()은 첫 줄만 꺼내고 두
#   번째 줄은 admin_buffer에 남는데, select()는 이미 다 읽어온 소켓에는 다시
#   신호를 안 주므로 이 두 번째 명령이 다음 관리자발 데이터가 도착할 때까지
#   ABB에 전혀 전달되지 않고 묻힐 수 있었다(직접 재현 테스트로 확인 -
#   scratchpad/connector_admin_drain_test.py). ABB -> 관리자 방향과 똑같은
#   방식으로 admin_buffer에 남은 줄을 전부 꺼내 전달하는 드레인 루프를
#   추가해서 고쳤다.
#
# V04 대비 이전 버전에서 바뀐 것 (관리자 재접속이 ABB 연결까지 끊던 버그 수정):
#   관리자 서버 연결이 끊기면(서버 재시작, 네트워크 순간 끊김 등) 이전에는
#   바깥 while True 루프를 통째로 다시 돌면서 finally에서 ABB 연결까지 같이
#   닫아버렸다. ABB(3abb.mod)는 아무 문제 없었는데 srv_client_socket이 갑자기
...
#   절대 건드리지 않는다.
#
# V03 대비 이 버전에서 바뀐 것 (수신 버퍼 드레인 버그 수정):
#   read_ready_line()은 한 번의 recv()로 완성된 줄을 하나만 꺼내 돌려줬는데,
#   3abb.mod가 이제 짧은 간격으로 여러 메시지(예: "ConveyDone" 직후 바로
...
#   ABB 쪽 RAPID 코드(3abb_V07.mod)가 작업 사이클 마지막에
...
#
# 역할:
#   1) ABB 로봇(3abb_V07.mod)에 클라이언트로 접속
...
def reconnect_admin(admin_host, admin_port, relay_name, board):
    """관리자 서버 재접속 - ABB 연결은 절대 건드리지 않는다.

    V05 이전에는 관리자 연결이 끊기면 바깥 while True 루프 전체를 다시 돌아서
    ABB 연결까지 같이 닫아버렸다. ABB(3abb.mod) 입장에서는 아무 문제도 없었는데
    srv_client_socket이 갑자기 끊긴 것처럼 보이고, WaitForAmrArrived의 ERROR
    핸들러는 ERR_SOCK_TIMEOUT만 처리하므로 이 ERR_SOCK_CLOSED는 위로
    전파되어 Run_Socket_Server까지 올라가 소켓을 다시 만들고 RETRY한다 - 그
    과정에서 몇 번째 차량을 조립 중이었는지 같은 진행 상태가 전부 날아가고
    Start 대기 상태로 리셋된다("두 번째 차량을 안 만들었다" 증상과 정확히
    일치). 관리자 쪽 연결 문제 때문에 아무 잘못 없는 ABB 세션을 끊을 이유가
    없으므로, 이제 관리자 연결만 따로 재접속한다."""
            ...
            while True:
                ...
                if admin_connection in readable:
                    ...
                    if command is not None:
                        ...
                        # v06: ABB -> 관리자 방향엔 진작 이런 드레인 루프가 있었는데
                        # (위 abb_connection 처리 참고), 관리자 -> ABB 방향엔 빠져
                        # 있었다. 한 번의 recv()에 명령이 여러 줄 섞여 들어오면(예:
                        # "Start" 직후 바로 다른 명령이 이어지는 경우) select()가
                        # 이미 다 읽어온 소켓엔 다시 신호를 안 주므로, admin_buffer에
                        # 남은 두 번째 줄이 다음 관리자발 데이터가 올 때까지 무한정
                        # 묻힐 수 있었다. 같은 방식으로 버퍼에 남은 줄을 전부 마저
                        # 꺼내 전달한다.
