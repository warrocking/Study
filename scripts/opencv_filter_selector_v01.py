...
class FilterSelector(tk.Tk):
    def __init__(self):
        ...
        self.title("OpenCV Filter Selector v0.1")
    ...
    def _build_ui(self):
        ...
        title = ttk.Label(
            root,
            text="OpenCV Filter Selector v0.1",
            font=("Arial", 18, "bold")
        )
        title.pack(anchor="w")
        ...
        subtitle = ttk.Label(
            root,
            text="체크박스로 원하는 필터를 선택한 뒤 Start를 누르세요.",
            font=("Arial", 10)
        )
        ...
        ttk.Button(bottom, text="추천 필터 선택", command=self.select_recommended).pack(side="left", padx=4)
        ttk.Button(bottom, text="전체 선택", command=self.select_all).pack(side="left", padx=4)
        ttk.Button(bottom, text="전체 해제", command=self.clear_all).pack(side="left", padx=4)
        ttk.Button(bottom, text="Start", command=self.start).pack(side="right", padx=4)
    ...
    def _build_param_panel(self, parent):
        frame = ttk.LabelFrame(parent, text="실행 파라미터", padding=10)
        frame.pack(fill="both", expand=True)

        devices = get_video_device_numbers()

        ttk.Label(frame, text="Camera Index").pack(anchor="w")
        ...
        self._add_entry(frame, "Tile Width", "tile_w")
        self._add_entry(frame, "Tile Height", "tile_h")
        self._add_entry(frame, "Columns", "cols")

        sep1 = ttk.Separator(frame, orient="horizontal")
        sep1.pack(fill="x", pady=8)

        self._add_entry(frame, "Canny Low", "canny_low")
        self._add_entry(frame, "Canny High", "canny_high")
        self._add_entry(frame, "Threshold Value", "threshold_value")
        self._add_entry(frame, "Min Contour Area", "min_contour_area")

        sep2 = ttk.Separator(frame, orient="horizontal")
        sep2.pack(fill="x", pady=8)

        ttk.Label(frame, text="HSV Mask Range").pack(anchor="w", pady=(0, 4))
        self._add_entry(frame, "H Low", "h_low")
        self._add_entry(frame, "H High", "h_high")
        self._add_entry(frame, "S Low", "s_low")
        self._add_entry(frame, "S High", "s_high")
        self._add_entry(frame, "V Low", "v_low")
        self._add_entry(frame, "V High", "v_high")

        sep3 = ttk.Separator(frame, orient="horizontal")
        sep3.pack(fill="x", pady=8)

        self._add_entry(frame, "HOG 실행 간격", "hog_every")
        self._add_entry(frame, "Motion Warn Ratio", "motion_warn_ratio")
        self._add_entry(frame, "Motion Pause Ratio", "motion_pause_ratio")
        self._add_entry(frame, "Edge Warn Ratio", "edge_warn_ratio")
        self._add_entry(frame, "Edge Pause Ratio", "edge_pause_ratio")
